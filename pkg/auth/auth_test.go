...
func TestTokenSource_WhenEmailFetchFailsOnFirstAcquisition_ItShouldNotCacheToken(t *testing.T) {
	// Email is empty (first acquisition for this TokenSource) and fetch fails —
	// the token must not be committed to the cache.
	f := &fakeFetcher{token: "new-token", emailErr: fmt.Errorf("no account")}
	ts := &TokenSource{fetcher: f}

	_, _, err := ts.Token(context.Background())
	if err == nil {
		t.Fatal("expected error when email fetch fails, got nil")
	}
	if ts.token != "" {
		t.Errorf("expected token not cached after email-fetch error, got %q", ts.token)
	}
}

func TestTokenSource_WhenEmailAlreadyCached_ItShouldNotRefetchOnTokenRefresh(t *testing.T) {
	// Email is already cached. Even though the email fetcher would return an
	// error, it should never be called during a token-only refresh.
	f := &fakeFetcher{
		token:    "fresh-token",
		emailErr: fmt.Errorf("should not be called"),
	}
	ts := &TokenSource{
		token:     "old-token",
	...
	if email != "cached@example.com" {
		t.Errorf("expected cached email unchanged, got %q", email)
	}
	if f.emailCallCount != 0 {
		t.Errorf("expected 0 email fetches when email is already cached, got %d", f.emailCallCount)
	}
}
...
func TestTokenSource_WhenCachedTokenExpiresWithinRefreshWindow_ItShouldRefetch(t *testing.T) {
	// A token whose expiry is within TokenEarlyRefresh (but not yet expired)
	// should be treated as stale and trigger a new fetch.
	f := &fakeFetcher{token: "new-token", email: "new@example.com"}
	ts := &TokenSource{
		token:     "stale-token",
		userEmail: "old@example.com",
		expiry:    time.Now().Add(TokenEarlyRefresh / 2), // inside the window
		fetcher:   f,
	}
	...
	if token != "new-token" {
		t.Errorf("expected refetch inside early-refresh window, got %q", token)
	}
	if f.callCount != 1 {
		t.Errorf("expected 1 fetch call, got %d", f.callCount)
	}
...
func TestTokenSource_WhenFetcherProvidesZeroExpiry_ItShouldUseDefaultLifetime(t *testing.T) {	f := &fakeFetcher{token: "gcloud-token", email: "user@example.com"} // zero expiry
	ts := &TokenSource{fetcher: f}
