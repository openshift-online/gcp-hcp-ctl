...
// All existing entries and fields in the kubeconfig are preserved.
//
...
func Update(opts UpdateOptions) (contextName, previousContext, kubeconfigPath string, err error) {
	...
	cluster.InsecureSkipTLSVerify = opts.InsecureSkipTLS

	authInfo := cfg.AuthInfos[opts.ClusterName]
	if authInfo == nil {
		authInfo = &clientcmdapi.AuthInfo{}
		cfg.AuthInfos[opts.ClusterName] = authInfo
	}
	authInfo.Exec = gcloudExecCredential()
...
// previousContext is empty the call is a no-op.
