...
func TestExecCredential(t *testing.T) {
	t.Run("When called it should return a gcphcpctl exec config with PlatformAPIAudience", func(t *testing.T) {
		exec := execCredential()
		...
		if exec.Args[3] != pkgauth.PlatformAPIAudience {
			t.Errorf("got Args[3] %q, want PlatformAPIAudience %q", exec.Args[3], pkgauth.PlatformAPIAudience)
		}
...
func TestUpdateExecCredentialAudience(t *testing.T) {
	t.Run("When Update is called it should write PlatformAPIAudience as the exec credential audience", func(t *testing.T) {
		dir := t.TempDir()
		path := filepath.Join(dir, "config")
		...
		// The audience must always be PlatformAPIAudience — the HyperShift
		// kube-apiserver OIDC config only accepts this fixed audience, not the
		// cluster API endpoint URL.
		args := user.Exec.Args
		if len(args) < 2 || args[len(args)-2] != "--audience" {
			t.Fatalf("expected --audience flag in args: %v", args)
		}
		if args[len(args)-1] != pkgauth.PlatformAPIAudience {
			t.Errorf("got audience %q, want PlatformAPIAudience %q", args[len(args)-1], pkgauth.PlatformAPIAudience)
		}
	})
}
