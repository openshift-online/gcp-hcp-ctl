...
// direct ID lookup, and if that returns 404, falls back to listing
// all clusters and matching by name.
...
func clusterStatus(c *hyperfleet.Cluster) string {
	...
	for _, cond := range c.Status.Conditions {
		if cond.Type == "Reconciled" {
			...
			if reason := ptrStr(cond.Reason); reason != "" {
				return reason
			}
		}
	}
